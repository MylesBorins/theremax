...
int Reverb::getInputRate(int channel) {
	int rate;
	switch (channel) {
		case 0: {
			rate = 1;
...
int Reverb::getOutputRate(int channel) {
	int rate;
	switch (channel) {
		case 0: {
			rate = 1;
...
void Reverb::classInit(int samplingFreq) {
	
};
	
void Reverb::instanceInit(int samplingFreq) {
...
void Reverb::init(int samplingFreq) {
	classInit(samplingFreq);
	instanceInit(samplingFreq);
};
	
void Reverb::buildUserInterface(UI* interface) {
	interface->openVerticalBox("reverb_designer");
	...
	interface->declare(&fhslider1, "tooltip", "This length (in meters) determines the shortest delay-line used in the FDN reverberator.     	      Think of it as the shortest wall-to-wall separation in the room.");
	interface->declare(&fhslider1, "unit", "m");
	interface->addHorizontalSlider("min acoustic ray length", &fhslider1, 46.f, 0.1f, 63.f, 0.1f);
	interface->declare(&fhslider2, "2", "");
	interface->declare(&fhslider2, "tooltip", "This length (in meters) determines the longest delay-line used in the FDN reverberator.     	      Think of it as the largest wall-to-wall separation in the room.");
	interface->declare(&fhslider2, "unit", "m");
	interface->addHorizontalSlider("max acoustic ray length", &fhslider2, 63.f, 0.1f, 63.f, 0.1f);
	interface->closeBox();
...
void Reverb::compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
