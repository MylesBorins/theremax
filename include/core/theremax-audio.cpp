...
RtAudio * TheremaxAudioIO::o_audio;
TheremaxAudioCallback TheremaxAudioIO::o_callback;
SAMPLE * TheremaxAudioIO::o_input_buffer;
SAMPLE * TheremaxAudioIO::o_output_buffer;
unsigned int TheremaxAudioIO::o_num_frames;
unsigned int TheremaxAudioIO::o_num_channels;
unsigned int TheremaxAudioIO::o_srate;
...
{
    // check status
    if( status ) cerr << "[theremax-audio]: overflow/underflow detected..." << endl;

    // call to TheremaxAudioIO
    return TheremaxAudioIO::cb( (SAMPLE *)outputBuffer, (SAMPLE *)inputBuffer, numFrames,
                         streamTime, data );
}
...
//-----------------------------------------------------------------------------
int TheremaxAudioIO::cb( SAMPLE * outputBuffer, SAMPLE * inputBuffer,
                  unsigned int numFrames, double streamTime, void * data )
{
    // check if callback
    if( !o_callback )
    {
        cerr << "[theremax-audio]: ERROR -- missing audio callback..." << endl;
        return 0;
    }
    ...
    {
        cerr << "[theremax-audio]: ERROR -- larger than expected frame size..." << endl;
        return 0;
    }
...
//-----------------------------------------------------------------------------
bool TheremaxAudioIO::init( unsigned int inputDevice,
                     unsigned int outputDevice,
                     unsigned int srate,
                     unsigned int & frameSize,
                     unsigned int numChannels,
                     TheremaxAudioCallback cb,
                     void * userData )
{
    ...
    {
        // error message
        cerr << "[theremax-audio]: already initialized..." << endl;
        return false;
    }
    ...
    try {
        ...
        try { // again
            ...
            // error message
            cerr << "[theremax-audio]: cannot initialize real-time audio I/O..." << endl;
            cerr << "[theremax-audio]: | - " << e.getMessage() << endl;
            // clean up
            SAFE_DELETE( o_audio );
            // done
            return false;
    ...
    {
        // error
        cerr << "[theremax-audio]: cannot allocate audio buffer(s)..." << endl;
        // clean up
...
//-----------------------------------------------------------------------------
bool TheremaxAudioIO::start()
{
    // check flag
    if( !o_audio )
    {
        cerr << "[theremax-audio]: trying to start uninitialized audio..." << endl;
        return false;
    }
    ...
    try {
        ...
        // error message
        cerr << "[theremax-audio]: cannot start real-time audio I/O..." << endl;
        cerr << "[theremax-audio]: | - " << e.getMessage() << endl;
        return false;
    }
...
//-----------------------------------------------------------------------------
void TheremaxAudioIO::stop()
{
    // check flag
    if( !o_audio )
    {
        cerr << "[theremax-audio]: trying to stop uninitialized audio..." << endl;
        return;
    }
